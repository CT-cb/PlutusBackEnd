...
describe('/all tests', () => {
    ...
    test('correct email param returns all expenses', async () => {
...
describe('/bydaterange tests', () => {
        ...
        test('incorrectStartDate returns all expenses from beginning of time to endDate', async () => {
            const res = await supertest(app)
                .get('/expenses/bydaterange')
                .query({
                    email: correctEmail,
    ...
    test('incorrectEndDate returns all expenses from startDate onward', async()=>{
        const res = await supertest(app)
            .get('/expenses/bydaterange')
            .query({
                email: correctEmail,
...
describe("byamount_daterange tests",()=>{
    test('no email in the request gives us an error',async()=>{
        const res = await supertest(app)
        .get("/expenses/byamount_daterange")
        .query({
            amountLowerBound: 10,
            amountUpperBound: 100
        });
